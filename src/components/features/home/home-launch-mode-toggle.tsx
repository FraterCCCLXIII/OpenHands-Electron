...
export const HOME_LAUNCH_MODES = ["code", "automate"] as const;

export type HomeLaunchMode = (typeof HOME_LAUNCH_MODES)[number];

interface HomeLaunchModeToggleProps {
  value: HomeLaunchMode;
  onChange: (value: HomeLaunchMode) => void;
  disabled?: boolean;
}
...
export function HomeLaunchModeToggle({
  value,
  onChange,
  disabled = false,
}: HomeLaunchModeToggleProps) {
  ...
  const options: Array<{
    ...
    {
      value: "automate",
      label: t(I18nKey.HOME$LAUNCH_MODE_AUTOMATE),
  ...
  return (
    ...
    >
      <LayoutGroup id="home-launch-mode">
        <div
          role="radiogroup"
          aria-label={t(I18nKey.HOME$LAUNCH_MODE_ARIA)}
          data-testid="home-launch-mode-toggle"
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                data-testid={`home-launch-mode-toggle-option-${option.value}`}
